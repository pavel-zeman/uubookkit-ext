...
(function () {
  ...
  /**
   * Returns true, if the element is a child of another element with editable content.
   * In such case, we should not run any specific actions for mouse click and keyboard events.
   */
  ...
  /**
   * Returns true, if the element is inside BUTTON, TEXTAREA, INPUT or IMG elements.
   * In this case, we should no run any specific actions for mouse click and keyboard events.
   */
  function isInForbiddenElementForEdit(element) {
  ...
  /**
   * Returns true, if the element is in a dropdown menu.
   */
  function isInDropDownMenu(element) {
    while (element) {
      if (element.classList.contains("uu5-bricks-dropdown-menu")) {
        return true;
      }
      element = element.parentElement;
    }
  ...
  function startEditMode(openSubsections) {
    ...
    if (activeElement.tagName !== "INPUT" && activeElement.tagName !== "TEXTAREA" && !isInEditableContent(activeElement)) { // Do not start edit mode, when typing text
      let forbiddenSections = {};
      // We do not want to edit the meaningless sections with buttons generated by business model kit or application model kit
      // These sections can be identified using blue button or search field
      $("div.uudcc-bricks-basic-section").each((index, item) => {
        if ($(item).find("button.color-schema-blue").length > 0 || $(item).find("input[name='search']").length > 0) {
          forbiddenSections[item.id + "-inner"] = true;
          forbiddenSections[item.id] = true;
        }
      });
      getUpdateButton().click();
      postStartEditMode(openSubsections, forbiddenSections);
    }
  }
   ...
   */
  function clickPencil(target) {
    let result = false;

    // Check, if the pencil can be clicked
    if (!isInForbiddenElementForEdit(target) && !isInEditableContent(target) && !isInDropDownMenu(target)) {
  ...
  let injectToHttpRequest = function () {
    ...
    unsafeWindow.fetch = async (...args) => {
      ...
      const url = args[0];
      if (url.includes("loadBook") || url.includes("loadPage") || url.includes("getBookStructure") || url.includes("listCommentThreads") || url.includes("sheet/load") || url.includes("sys/uuAppWorkspace/load")) {
        response.clone().json()
          .then(body => {
  ...
  // Adds timer to enable spellcheck for rich text editor
  setInterval(() => {
    document.querySelectorAll("div[spellcheck='false']").forEach(div => div.spellcheck = true);
  }, 1000);
  ...
  window.addEventListener("click", (e) => {
    if (isInEditMode()) {
      ...
      if (searchAndClickPencil && !isInEditableContent(e.target)) {
        clickPencil(e.target);
      }
    }
  });
